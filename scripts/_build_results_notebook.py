...
CELLS = [
    ...
    new_markdown_cell(
        ...
        "afternoon peak). Ridge with only first-harmonic cyclic features "
        "could not represent two peaks per day and lost to the trivial "
        "hour-of-day average; the feature experiment "
        "(`docs/experiments/2026-06-01_leakage-safe-feature-sweep.md`) added "
        "a second hour harmonic and workingday-gated cyclic terms and lifted "
        "Ridge just past that baseline (0.91 -> 0.72), an explainability "
        ...
        "\n"
        "**Model choice.** XGBoost leads the day-of-month holdout (the "
        "primary view), while Gradient Boosting is marginally ahead on the "
        "chronological CV (0.453 vs 0.463) and Random Forest sits just "
        "behind; the three should be read as a near-tie rather than a "
        "decisive winner from one metric. The hour-of-day mean is a "
        "remarkably strong, interpretable baseline: the engineered Ridge "
