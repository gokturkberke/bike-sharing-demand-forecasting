...
CELLS = [
    ...
    new_markdown_cell(
        ...
        "\n"
        "- The cyclic hour encoding places hour 23 next to hour 0 on the "
        "unit circle (verified visually and by the `test_cyclic_encoding_"
        "continuity` test). This lets a linear model treat \"close hours\" "
        "as close inputs without needing a one-hot expansion.\n"
        "- The hour × workingday interaction survives feature engineering "
        "unchanged. Tree models can pick this up from raw `hour` + "
        "`workingday`; linear models will need an explicit interaction "
        "term in Phase 4 or rely on the cyclic encoding plus `workingday`.\n"
        "- Among the new features, `hour`, `year`, and `month`-related "
        "cyclic columns show the strongest single-feature correlations "
        "with `count`. `is_weekend` is weakly correlated because the "
        "working-day signal already captures most weekend behavior."
    ),
]
