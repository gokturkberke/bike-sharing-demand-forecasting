...
- This is a CMP4336 school project (not a leaderboard run); the emphasis is EDA, feature engineering, modeling, validation, and explainable reporting. The repository is currently at Phase 5: in addition to the Phase 1-4 deliverables (package scaffold, raw-data loading, configuration validation, leakage preprocessing, feature engineering producing `data/processed/{train,test}.parquet`, the executed EDA + feature-engineering + baseline/linear notebooks, and the baseline/Ridge modeling pipeline), tree models are online. `src/bike_sharing/evaluate.py` computes RMSLE, RMSE, MAE, R² and reports them together (no single metric is primary). `src/bike_sharing/models.py` provides a factory for `mean_baseline`, `hourly_mean_baseline`, `ridge`, `random_forest`, and `gradient_boosting`; every real model trains on `log1p(count)` and inverts with `from_log1p` (expm1 + clip-at-0) so predictions can never be negative. The Ridge is a `ColumnTransformer + Ridge` that drops raw ordinal time columns and one-hots `season`/`weather`; the tree models are scale-invariant and use the full feature set (raw ordinals kept). `get_model(name, cfg, params)` takes hyperparameters from `config/models.yaml` (loaded by `config.load_models_config`); `random_state` comes from `cfg.seed`, not models.yaml. `src/bike_sharing/train.py` provides two leakage-safe validation views: `fit_and_cv` (`TimeSeriesSplit(n_splits=cfg.cv.n_splits)`, rows sorted by datetime) and `evaluate_holdout` (a day-of-month split via `day_of_month_holdout_split` - train days 1-15, validate days 16-19, mirroring the dataset's own 1-19/20+ structure since labeled data stops at day 19). `cv` is a required config key validated in `config.py`. `scripts/train_model.py --model NAME` persists `models/{name}.joblib` and writes both views into `reports/metrics.json` under `{cv, day_of_month_holdout}`. `notebooks/04_tree_models.ipynb` adds model comparison and feature-importance figures (`reports/figures/12-13`). Tests cover the metric, model-factory, models.yaml, and validation contracts. Current day-of-month-holdout RMSLE: `random_forest` 0.33 (R² 0.92), `gradient_boosting` 0.33 (R² 0.90), `hourly_mean_baseline` 0.75, `ridge` 0.91, `mean_baseline` 1.53 - the trees capture the hour x workingday interaction the linear model could not, and feature importance is dominated by the temporal signal with environmental inputs secondary.
