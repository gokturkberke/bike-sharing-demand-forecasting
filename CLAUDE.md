...
- This is a CMP4336 school project (not a leaderboard run); the emphasis is EDA, feature engineering, modeling, validation, and explainable reporting. The repository is at Phase 7 (final): the modeling pipeline (baselines, Ridge, RF, GBM, XGBoost), the `datetime,count` prediction-artifact pipeline, and the five-notebook analysis are all in place, and the consolidated results report is written. `notebooks/05_results_and_interpretation.ipynb` plus `reports/RESULTS.md` are the headline deliverable: they pull the full metric comparison, the best model's out-of-sample error analysis (figures 14-16), and the feature-importance story together and read them against the proposal's environmental, temporal, and sequential-data questions (target-lag features are deliberately excluded for leakage safety). The earlier-phase machinery is unchanged: `src/bike_sharing/evaluate.py` computes RMSLE, RMSE, MAE, R² and reports them together (no single metric is primary). `src/bike_sharing/models.py` provides a factory for `mean_baseline`, `hourly_mean_baseline`, `ridge`, `random_forest`, `gradient_boosting`, and `xgboost` (imported lazily so it stays an optional dependency); every real model trains on `log1p(count)` and inverts with `from_log1p` (expm1 + clip-at-0) so predictions can never be negative. The Ridge is a `ColumnTransformer + Ridge` that drops raw ordinal time columns and one-hots `season`/`weather`; the tree/boosting models are scale-invariant and use the full feature set (raw ordinals kept). `get_model(name, cfg, params)` takes hyperparameters from `config/models.yaml` (loaded by `config.load_models_config`); `random_state` comes from `cfg.seed`, not models.yaml. `src/bike_sharing/train.py` provides two leakage-safe validation views: `fit_and_cv` (`TimeSeriesSplit(n_splits=cfg.cv.n_splits)`, rows sorted by datetime) and `evaluate_holdout` (a day-of-month split via `day_of_month_holdout_split` - train days 1-15, validate days 16-19, mirroring the dataset's own 1-19/20+ structure since labeled data stops at day 19). `src/bike_sharing/predict.py` (`make_prediction_frame`, `write_prediction_artifact`) builds the `datetime,count` artifact preserving test row order; `scripts/generate_submission.py --model NAME` loads `models/{name}.joblib` + `data/processed/test.parquet` and writes `reports/submissions/{name}_{timestamp}.csv` (gitignored). `cv` is a required config key validated in `config.py`. `scripts/train_model.py --model NAME` persists `models/{name}.joblib` and writes both views into `reports/metrics.json` under `{cv, day_of_month_holdout}`. Tests cover metric, model-factory, models.yaml, validation, and prediction-artifact-schema contracts. Current day-of-month-holdout RMSLE: `xgboost` 0.31 (R² 0.94, best), `random_forest` 0.33 (R² 0.92), `gradient_boosting` 0.33 (R² 0.90), `hourly_mean_baseline` 0.75, `ridge` 0.91, `mean_baseline` 1.53 - the trees capture the hour x workingday interaction the linear model could not; the three trees are close, with XGBoost narrowly ahead.
