...
- **DONE (commit `87f844e`):** Added `comfort_index` to `build_candidate_features` and `CANDIDATE_NUMERIC_COLUMNS` (production `build_features` / `ADDED_FEATURE_COLUMNS` untouched); added a `test_features.py` check that it is finite, varies, and is candidate-only. `pytest` green (93 passed).
...
- **DONE (commit `87f844e`):** Ran `scripts/run_env_experiment.py`; the baseline arm reproduced `reports/metrics.json` holdout exactly for all four models (harness valid).
